...
- Evaluate using the same evidence rules as Mode D/E, with the same two-phase model: Phase 1 reads using question, reader_role, document body, and structural frontmatter fields — but not `tddoc.tests_inline` or `tddoc.evaluation`; Phase 2 grading pass applies `expected_answer_properties` and `failure_risk`.
...
Evaluate the full corpus for the set-level tests using the same two-phase model (read pass first, rubric grading second; `tddoc.tests_inline` and `tddoc.evaluation` excluded from Phase 1, structural frontmatter available). Record:
