...
- Evaluate using the same evidence rules as Mode D/E, with the same two-phase model: Phase 1 reads using only question, reader_role, and document body (do not use `tddoc` frontmatter); Phase 2 grading pass applies `expected_answer_properties` and `failure_risk`.
...
Evaluate the full corpus for the set-level tests using the same two-phase model (read pass first, rubric grading second; `tddoc` frontmatter excluded from Phase 1). Record:
